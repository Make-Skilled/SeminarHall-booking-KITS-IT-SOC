...
def adding_ground():
    ...
    file = request.files.get("groundImage")
    if file and allowed_file(file.filename):
        filename = secure_filename(file.filename)
        file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename))
        f = os.path.join(app.config['UPLOAD_FOLDER'], filename)  # Path to the saved file
    else:
        return render_template("addground.html", status="Invalid image file!")
    ...
    return render_template("addground.html", status="Hall uploaded successfully!")
